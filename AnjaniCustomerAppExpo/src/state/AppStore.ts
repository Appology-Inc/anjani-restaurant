...
export interface ActiveOrder {
  ...
  userLat: number;
  userLng: number;
  customerUid?: string; // Auth UID attached to order
  ...
  return {
    ...
    placeOrder: (address, phone, paymentMethod, utrNumber, cookingInstructions, userLat, userLng) => {
      ...
      const newOrder: ActiveOrder = {
        id: orderId,
        customerUid: auth?.currentUser?.uid || 'anonymous-fallback',
        items: orderItems,
        totalAmount,
    ...
    sendChatMessage: (orderId, senderRole, senderName, text) => {
      ...
      if (isFirebaseConfigured) {
        try {
          ...
          addDoc(msgRef, newMsg)
            .catch((e: any) => console.log('Firestore chat message sync deferred:', e.message));
        } catch (e: any) {
          console.log('Firebase chat message skipped:', e.message);
        }
      }
    },
...
// Setup Firebase Anonymous Auth for Customer
if (isFirebaseConfigured && auth) {
  ...
  onAuthStateChanged(auth, (user) => {
    if (user) {
      console.log("Customer authenticated anonymously with UID:", user.uid);
    }
  });
}
