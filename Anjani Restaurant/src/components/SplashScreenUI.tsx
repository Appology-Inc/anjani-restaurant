/**
 * @file SplashScreenUI.tsx
 * @description A highly visual, cinematic splash screen for the application.
 * Features complex continuous background animations and a looping delivery track animation.
 */
 ...
 * 
 * A self-contained micro-animation showing the journey of a food order.
 * It animates an icon along a track, switching the icon from 'food' to 'rider' to 'home'
 * at specific intervals to represent the delivery process.
 * 
 * @returns {React.JSX.Element} The animated track component.
 */
function AnimatedDeliveryTrack() {
  // Controls the physical movement of the icon along the track (0 to 1)
  const travelAnim = useRef(new Animated.Value(0)).current;
  // Controls which icon is currently displayed based on the animation progress
  const [iconPhase, setIconPhase] = useState<'food' | 'rider' | 'home'>('food');

  useEffect(() => {
    // Duration for one complete cycle of the delivery animation
    const duration = 2000;
    const runAnim = () => {
...
/**
 * SplashScreenUI Component
 * 
 * The main splash screen view. It orchestrates a slow, cinematic "Ken Burns" style
 * panning and scaling animation on the background image, while fading in the foreground
 * title, branding, and the AnimatedDeliveryTrack.
 * 
 * @returns {React.JSX.Element} The complete splash screen view.
 */
export default function SplashScreenUI() {
  // Controls the fade-in of the foreground text and UI elements
  const opacity = useRef(new Animated.Value(0)).current;
  // Controls the slow scaling (zoom) effect of the background image
  const imageScale = useRef(new Animated.Value(1)).current;
  // Controls the slow panning effect of the background image
  const imageTranslateX = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    // Initial fade-in of the foreground UI elements
    ...
    // Continuous loop of parallel animations for the cinematic background effect
    Animated.loop(
      Animated.parallel([
        Animated.sequence([
