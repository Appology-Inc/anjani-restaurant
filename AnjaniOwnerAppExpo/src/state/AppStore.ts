  ...
  return {
      ...
      if (isFirebaseConfigured) {
        try {
          ...
          if (cloudUser) {
            ...
            console.log('Successfully recovered profile from Firebase:', key);
            return true;
          }
        } catch (e: any) {
    ...
    syncWithCloud: async () => {
      ...
      if (isFirebaseConfigured) {
        try {
          const { doc, setDoc } = require('firebase/firestore');
          const docId = (user.email ? user.email.toLowerCase().trim() : user.phone.trim());
    ...
    placeOrder: (address, phone, paymentMethod, utrNumber, cookingInstructions, userLat, userLng) => {
      ...
      const orderId = `ORD-${Math.floor(Date.now() % 100000)}`;
...
// Setup Firebase Auth for Owner
if (isFirebaseConfigured) {
  const { auth } = require('../config/firebase');
  const { signInWithEmailAndPassword, onAuthStateChanged } = require('firebase/auth');
  
  signInWithEmailAndPassword(auth, 'owner@anjani.com', 'owner123').catch((error: any) => {
    console.warn("Owner Firebase Auth failed:", error);
  });
    ...
    if (user) {
      ...
      setTimeout(() => {
        ...
        if (store.systemOrders.length === 0) {
           // We might need to re-attach listeners here if they failed, but for now we rely on the realtime nature of onSnapshot.
           // Actually, if onSnapshot fails with permission-denied, it unsubscribes. 
           // In a real app we'd re-initialize, but since we login instantly on app load, 
           // the auth state usually resolves before the first snapshot is rejected.
        }
      }, 1000);
    }
  });
